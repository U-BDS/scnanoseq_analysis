...
* Reference information: hg38 GENCODE XXXX (# TODO add details with reference matching Illumina run)
...
```{r, eval=FALSE}
hg38_q <- query(ah, c("Ensembl", "Homo sapiens","106")) ##TODO change to final version we settle (matching original Illumina run)
hg38_q

hg38_q$title # finding the same gtf we used for generating 10X reference

hg38_annot <- subset(hg38_q, title == "Homo_sapiens.GRCh38.106.gtf")[[1]] ##TODO change to final version we settle
...
```{r}
#TODO: consider making a log file of Ensemlb and names matched for easy look-up back to ids if needed
cell_bc_matrix_genes <- mapply(FUN = function(x,y) {
...
}, x=cell_bc_matrix, y=feature_type_scnanoseq, SIMPLIFY = FALSE)
```
...
We generate these prior to any filtering, since filtering based on cutoff are bound to vary on technological differences and other factors (e.g. see note below). Depending on this, the levels of filtration will vary (and downstream correlations will be limited to cells retained in both datasets. But here we show raw as well)

NOTE: upon investigation of what genes are detected (using feature names, rather than pattern), oxford/scnanoseq samples contained a lot more MT genes than Illumina/CR (TODO: add a table showing this, as supplementary?). I also note, several cells here have high MT rates.
...
Unlike with the raw data, here we only investigate pattern across common cells retained due to differences in the effect that MT filtering is causing due to higher MT genes detected in Oxfords vs Illumina.
...
First slimming down the object as Azimuth performs normalization internally on RNA assay, run Azimuth and save metadata to be imported into objects
...
mapply(FUN = function(x) {
  ...
  if (x %in% rownames(seurat_int$SC3pv3_gene_int@assays$SCT@counts)) {
    ...
    png(paste0(output_prefix, "/feature_expression/",x, "_pbmc_gene.png"),width=500,height=500)
    plot(p1)
    dev.off()
...
mapply(FUN = function(x) {
  ...
  if (x %in% rownames(seurat_int$SC5pv2_gene_int@assays$SCT@counts)) {
    ...
    png(paste0(output_prefix, "/feature_expression/",x, "_lung_gene.png"),width=500,height=500)
    plot(p1)
    dev.off()
